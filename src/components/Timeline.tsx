        ...
        <div className="flex flex-col gap-16 md:gap-24 w-full">
          {steps.map((step, idx) => {
            ...
            return (
              <div 
                key={idx}
                className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center w-full relative z-10"
              >
                
                {/* 1. TEXT COLUMN (Left on even, Right on odd) */}
                <div className={`col-span-1 md:col-span-5 flex flex-col gap-4 text-left ${
                  isEven ? "md:order-1 md:text-right md:items-end" : "md:order-3 md:text-left md:items-start"
                }`}>
                  <motion.span 
                ...
                {/* 2. CENTRAL BULB NODE */}
                <div className="absolute left-6 md:left-1/2 top-0 md:top-1/2 -translate-x-1/2 -translate-y-1/2 md:order-2 z-20 flex items-center justify-center">
                  <div className="relative">
                    {/* Breathing animated aura */}
                    <div 
                ...
                {/* 3. VISUAL PREVIEW BLOCK (Right on even, Left on odd) */}
                <div className={`col-span-1 md:col-span-6 flex justify-center items-center ${
                  isEven ? "md:order-3 md:pl-8" : "md:order-1 md:pr-8"
                }`}>
                  <motion.div 
                  ...
                  >
                    ...
                    <div 
                      className={`absolute -top-16 -right-16 h-36 w-36 rounded-full opacity-[0.03] group-hover/mock:opacity-[0.08] transition-opacity duration-700 blur-2xl pointer-events-none ${colors.bg}`}
                    />

                    {/* Step Visual renderers */}
                    ...
                    {step.visual === "github" && (
                      <div className="flex flex-col gap-3 w-full max-w-xs mx-auto text-left">
                        ...
                        {/* Git Telemetry Micro Heatmap Grid */}
